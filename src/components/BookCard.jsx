...
const BookCard = ({ book }) => {
  const { removeBook, changeReadState } = useContext(BookContext);
  const handleRemove = () => {
    removeBook(book.id);
  };
  const handleToRead = () => {
    changeReadState(book.id);
  };
  ...
  return (
    <div className="book-card">
      ...
      <div className="book-actions">
        <button className="button is-primary is-squre" onClick={handleToRead}>
          {book.toRead ? (
            <svg
        ...
        <button className="button is-danger is-squre" onClick={handleRemove}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="24px"
            viewBox="0 0 24 24"
